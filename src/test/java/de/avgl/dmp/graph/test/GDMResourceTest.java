...
public class GDMResourceTest {
	...
	public void testPingToTestDB() throws IOException {
		...
		final NeoServer server = CommunityServerBuilder.server().onPort(7499)
				.withThirdPartyJaxRsPackage("de.avgl.dmp.graph.resources", GDMResourceTest.MOUNT_POINT).build();
		server.start();

		final Client c = Client.create();
		final WebResource service = c.resource(server.baseUri().resolve(GDMResourceTest.MOUNT_POINT));
