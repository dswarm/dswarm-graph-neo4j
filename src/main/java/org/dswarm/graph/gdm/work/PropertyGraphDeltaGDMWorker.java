 ...
 * @author tgaengler
 */
public class PropertyGraphDeltaGDMWorker implements GDMWorker {

	private static final Logger			LOG							= LoggerFactory.getLogger(PropertyGraphDeltaGDMWorker.class);
	...
	public PropertyGraphDeltaGDMWorker(final String resourceUriArg, final GraphDatabaseService databaseArg) {
	...
	public void work() {
		...
		PropertyGraphDeltaGDMWorker.LOG.debug("start read GDM TX");

		try {

			final Label recordClassLabel = DynamicLabel.label("__LEAF__");

			final ResourceIterable<Node> leafNodes = database.findNodesByLabelAndProperty(recordClassLabel, "__LEAF__",
					true);
			...
			PropertyGraphDeltaGDMWorker.LOG.error("couldn't finished read GDM TX successfully", e);

			tx.failure();
			tx.close();
		} finally {

			PropertyGraphDeltaGDMWorker.LOG.debug("finished read GDM TX finally");
				...
				switch (subjectNodeType) {
					...
					case TypeResource:
						...
						if (subjectURI == null) {
							...
							PropertyGraphDeltaGDMWorker.LOG.error(message);
