...
public class FullRDFExportMultipleGraphsOnRunningDBTest extends FullRDFExportOnRunningDBTest {
	...
	public void readAllRDFFromRunningDB() throws IOException {
		...
		FullRDFExportMultipleGraphsOnRunningDBTest.LOG.debug("start export all RDF statements test for RDF resource at running DB");
		...
		writeRDFToRunningDBInternal(provenanceURI1);
		writeRDFToRunningDBInternal(provenanceURI2);
		...
		FullRDFExportMultipleGraphsOnRunningDBTest.LOG.trace("Response body : " + body);
		...
		FullRDFExportMultipleGraphsOnRunningDBTest.LOG.debug("exported '" + statementsInExportedRDFModel + "' statements");
		...
		modelFromOriginalRDFile2.read(Resources.getResource(FullRDFExportOnRunningDBTest.TEST_RDF_FILE).getFile(), "TURTLE");
		...
		FullRDFExportMultipleGraphsOnRunningDBTest.LOG.debug("finished export all RDF statements test for RDF resource at running DB");
	}
}
