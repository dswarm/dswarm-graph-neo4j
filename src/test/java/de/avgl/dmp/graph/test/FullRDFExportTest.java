...
public class FullRDFExportTest {
	...
	public void readAllRDFFromTestDB() throws IOException {
		...
		final NeoServer server = CommunityServerBuilder
				.server()
				.onPort(7499)
				.withThirdPartyJaxRsPackage("de.avgl.dmp.graph.resources",
						RDFResourceTest.MOUNT_POINT).build();
		server.start();

		writeRDFToTestDBInternal(server);

		final Client c = Client.create();
		final WebResource service = c.resource(server.baseUri().resolve(
				RDFResourceTest.MOUNT_POINT));
	...
	public void testPing() throws IOException {
		...
		// POST the request
		final ClientResponse response = service.path("/rdf/ping").get(
	...
	private void writeRDFToTestDBInternal(final NeoServer server)
			throws IOException {
		...
		multiPart.bodyPart(
				...
				.bodyPart(
						new BodyPart("http://data.slub-dresden.de/resources/1",
								MediaType.TEXT_PLAIN_TYPE));

		// POST the request
		final ClientResponse response = service.path("/rdf/put")
